    ...
    window.TWAttack = {
        ...
        utils: {
            ...
            getCurrentVillageCoords: function() {
                ...
                // Method 1: Extract from URL hash (e.g., #480;475)
                var hash = window.location.hash;
                if (hash) {
                    // Remove the # symbol
                    var coordsStr = hash.substring(1);
                    // Check for format like "480;475" or "480|475"
                    var match = coordsStr.match(/(\d+)[;|](\d+)/);
                    if (match) {
                        var coords = match[1] + "|" + match[2];
                        console.log('TW Attack: Coordinates extracted from URL hash:', coords);
                        window.TWAttack.state.homeCoords = coords;
                        return coords;
                    }
                }
                
                // Method 2: Extract from full URL (in case hash is not present)
                var url = window.location.href;
                ...
                // Method 3: Extract from page title (fallback method)
                var title = document.querySelector('head > title');
                if (title) {
                    var titleMatch = title.textContent.match(/\((\d+)\|(\d+)\)/);
                    if (titleMatch) {
                        var coords = titleMatch[1] + "|" + titleMatch[2];
                        console.log('TW Attack: Coordinates extracted from page title:', coords);
                        window.TWAttack.state.homeCoords = coords;
                        return coords;
                    }
                }
                
                console.log('TW Attack: Could not extract coordinates from URL or title');
                return "";
            },
            ...
            // New function to parse coordinates from any string
        ...
        initialize: function() {
            ...
            console.log('TW Attack: URL hash:', window.location.hash);
